...
A browser extension that automatically suspends inactive tabs to free up memory and CPU, with configurable rules, whitelist support, and smart activity detection.
...
Built on the standard WebExtensions API (Manifest V3). Works on any Chromium-based browser that supports the extensions platform, including but not limited to Chrome, Vivaldi, Edge, and Brave.
...
- **Two suspension modes:** replace the tab with a lightweight page, or use the browser's native discard API
- **Activity-aware:** resets the inactivity timer on clicks, keypresses, and scrolls; ignores mouse movement
- **Rule-based control:** define which tabs to suspend using URL patterns or tab group names, each with their own delay
...
2. Open your browser's extensions page (usually accessible via the browser menu)
